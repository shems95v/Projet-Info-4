/**
 * filtres.js — Filtres et tris dynamiques de la page présentation
 *
 * Fonctionnement :
 *  - Les tris (prix, nom) opèrent sur les données déjà chargées en mémoire (côté client).
 *  - Les filtres (type, saveur, diététique) déclenchent une requête asynchrone vers api_plats.php.
 *  - La barre de recherche filtre côté client en temps réel sur les données actuelles.
 */
...
const compteurEl    = document.getElementById('compteur-plats');   // optionnel
const inputRecherche = document.getElementById('recherche-plats'); // barre de recherche
...
function afficherPlats(plats) {
    ...
    /* Mémoriser pour le tri côté client */
    platsCourants = plats;
}
...
/* =========================================================
   Requête asynchrone vers api_plats.php (pour les filtres)
   ========================================================= */
async function fetchPlats() {
    const params = collecterFiltres();

    /* Ajouter le tri dans la requête si le serveur peut l'appliquer directement */
...
/* Recherche → requête serveur (avec debounce) */
if (inputRecherche) {
    inputRecherche.addEventListener('input', fetchDebounce);
}

/* Tri → côté client uniquement (données déjà en mémoire) */
if (selectTri) {
