...
if($_SERVER["REQUEST_METHOD"] === "POST"){
    if($password_confirm != $password){
        $verife_mdp = "Les mots de passe sont différents";
    }
    // Vérification que l'email, le login et le téléphone ne sont pas déjà pris
    foreach($backdata as $key){
        if($key["email"] == $email){
            $verife_email = "Email déjà utilisé";
            break;
        }
        if($key["login"] == $login){
            $verife_log = "Login déjà utilisé";
            break;
        }
        if($key["telephone"] == $telephone){
            $verife_tel = "Numéro déjà utilisé";
            break;
        }
    }
    if(!empty($telephone) && substr($telephone, 0, 2) != "06" && substr($telephone, 0, 2) != "07"){
        $verife_tel = "Numéro invalide";
    }
}

if(empty($verife_email) && empty($verife_tel) && empty($verife_mdp) && empty($verife_log)){
    if(!empty($nom) && !empty($prenom) && !empty($telephone) && !empty($email) && !empty($adresse) && !empty($code_postal) && !empty($ville) && !empty($password) && !empty($password_confirm) && !empty($login)){
        $backdata[] = array(
            'id' => uniqid(),
        ...
        <div class="conteneur-formulaire">
            ...
            <form action="inscription.php" method="POST">
                ...
                <div class="groupe-formulaire">
                    ...
                    <input type="text" id="login" name="login" maxlength="12" value="<?php echo htmlspecialchars($login); ?>" placeholder="Jean" required>
                    <small id="msg-login"></small>
                    <?php if(!empty($verife_log)): ?>
                        <p style="color:red; margin-bottom:15px; text-align:center;"><?php echo $verife_log; ?></p>
                    <?php endif; ?>
                </div>

                <div class="groupe-formulaire">
                    <label for="email">Adresse email <span class="obligatoire">*</span></label>
                    <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($email); ?>" placeholder="votre.email@exemple.com" required>
                    <?php if(!empty($verife_email)): ?>
                        <p style="color:red; margin-bottom:15px; text-align:center;"><?php echo $verife_email; ?></p>
                    <?php endif; ?>
                </div>
                ...
                <div class="groupe-formulaire">
                    ...
                    <input type="tel" 
                        ...
                        required>
                    <?php if(!empty($verife_tel)): ?>
                        <p style="color:red; margin-bottom:15px; text-align:center;"><?php echo $verife_tel; ?></p>
                    <?php endif; ?>
                </div>
                ...
                <div class="groupe-formulaire">
                    ...
                    <?php if(!empty($verife_mdp)): ?>
                        <p style="color:red; margin-bottom:15px; text-align:center;"><?php echo $verife_mdp; ?></p>
                    <?php endif; ?>
                </div>
                ...
                <div class="groupe-formulaire">
                    ...
                    <!-- Champ optionnel pour les allergies etc. -->
                    <textarea id="commentaire" name="commentaire" placeholder="Allergies, préférences de livraison..."><?php echo htmlspecialchars($commentaire); ?></textarea>
                </div>
