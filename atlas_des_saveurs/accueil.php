<?php
session_start();
?>

<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>L'Atlas Des Saveurs</title>
    <link id="theme-style" rel="stylesheet" href="accueil.css">
    <script src="modeSombre.js"></script>
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@600&family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet">
</head>

<body>

<header>
    <h1>L'Atlas Des Saveurs</h1>
    <button id="btn-dark-mode">Changer thème</button>
    <nav class="navigation">
        <?php
        // La nav change selon le rôle
        if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] === 'admin') {
            echo '<a href="admin.php">Admin</a>';
            echo '<a href="livraison.php">Livraison</a>';
            echo '<a href="commandes.php">commandes</a>';
        }
        if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] === 'livreur') {
            echo '<a href="livraison.php">Livraison</a>';
        }
        if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] === 'restaurateur') {
            echo '<a href="restaurateur.php">Gestion commandes</a>';
        }
        ?>
        <a href="accueil.php">Accueil</a>
        <a href="presentation.php">Menu</a>
        <?php
        // Si pas connecté on montre Inscription/Connexion, sinon les liens du compte
        if (!isset($_SESSION['user'])) {
            echo '<a href="inscription.php">Inscription</a>';
            echo '<a href="connexion.php">Connexion</a>';
        } else {
            echo '<a href="profil.php">Mon profil</a>';
            echo '<a href="panier.php">Panier</a>';
            echo '<a href="logout.php">Déconnexion</a>';
        }
        ?>
    </nav>
    
</header>

<main>

    <p class="intro">
        Bienvenue à <strong>L'Atlas Des Saveurs</strong> ! Plongez dans un voyage culinaire unique où chaque plat est préparé avec passion et authenticité. Nos plats sont conçus pour éveiller vos sens et ravir vos papilles.
    </p>

    <h4>Nos formules</h4>

    <div class="plats">

        <!-- Plat du jour : en dur pour l'instant, idéalement à dynamiser depuis le JSON des plats -->
        <div class="colonne">
            <h3>Plat du jour</h3>
            <ul>
                <li>
                    <a href="#" class="carte-plat carte-plat-grande carte-plat-katsu">
                        <span class="nom-plat">Katsu Curry</span>
                        <span class="prix-plat">14,50 €</span>
                    </a>
                </li>
            </ul>
        </div>

        <!-- Souvent commandés : aussi en dur, à brancher sur nbCommandes du JSON si on veut du dynamique -->
        <div class="colonne">
            <h3>Souvent commandés</h3>
            <ul>
                <li>
                    <a href="#" class="carte-plat carte-plat-gyozas">
                        <span class="nom-plat">Gyozas Maison</span>
                        <span class="prix-plat">8,00 €</span>
                    </a>
                </li>
                <li>
                    <a href="#" class="carte-plat carte-plat-baklava">
                        <span class="nom-plat">Baklava au Miel</span>
                        <span class="prix-plat">7,00 €</span>
                    </a>
                </li>
                <li>
                    <a href="#" class="carte-plat carte-plat-pastilla">
                        <span class="nom-plat">Pastilla au Poulet</span>
                        <span class="prix-plat">9,50 €</span>
                    </a>
                </li>
            </ul>
        </div>

    </div>

    <div class="voir-menu">
        <a href="presentation.php">Voir tout le menu →</a>
    </div>

</main>

<footer>
    <p>&copy; 2026 L'Atlas Des Saveurs — Tous droits réservés</p>
</footer>

</body>
</html>